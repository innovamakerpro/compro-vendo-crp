...
import { actualizarVehiculo, eliminarImagen } from "./actions";
...
export default function EditarVehiculoForm({ vehiculo, imagenes: imagenesIniciales }: Props) {
  const [state, action, isPending] = useActionState(actualizarVehiculo, null);
  ...
  function handleEliminarImagen(imagen: ImagenExistente) {
    if (!confirm("¿Eliminar esta imagen? Esta acción no se puede deshacer.")) return;
    startDeleting(async () => {
      await eliminarImagen(imagen.id, vehiculo.id, imagen.url);
      setImagenes((prev) => prev.filter((img) => img.id !== imagen.id));
      router.refresh();
    });
  }

  // Precios en euros para el formulario (en BD están en céntimos)
  const precioEuros = (vehiculo.precio / 100).toFixed(2);
  const precioAntEuros = vehiculo.precio_anterior
    ? (vehiculo.precio_anterior / 100).toFixed(2)
    : "";
    ...
    <form action={action} className="max-w-4xl">
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select name="marca" required className={FIELD} defaultValue={vehiculo.marca}>
          ...
          <div>
            ...
            <input name="modelo" required defaultValue={vehiculo.modelo} placeholder="Ej: Serie 3, Golf, Tucson…" className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Versión</label>
            <input name="version" defaultValue={vehiculo.version ?? ""} placeholder="Ej: 320d M Sport, 1.5 TSI…" className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Año <span className="text-red-400">*</span></label>
            <input name="anio" type="number" required min="1900" max="2100" defaultValue={vehiculo.anio} className={FIELD} />
          </div>
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select name="combustible" required className={FIELD} defaultValue={vehiculo.combustible}>
          ...
          <div>
            ...
            <select name="cambio" required className={FIELD} defaultValue={vehiculo.cambio}>
              <option value="" disabled>Selecciona</option>
          ...
          <div>
            ...
            <select name="carroceria" required className={FIELD} defaultValue={vehiculo.carroceria}>
              <option value="" disabled>Selecciona</option>
          ...
          <div>
            ...
            <input name="kilometraje" type="number" required min="0" defaultValue={vehiculo.kilometraje} className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Potencia (CV)</label>
            <input name="potencia" type="number" min="1" defaultValue={vehiculo.potencia ?? ""} className={FIELD} />
          </div>
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input name="color" defaultValue={vehiculo.color ?? ""} placeholder="Ej: Negro Zafiro" className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Garantía</label>
            <input name="garantia" defaultValue={vehiculo.garantia ?? ""} placeholder="Ej: 12 meses incluida" className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Plazas</label>
            <input name="plazas" type="number" min="1" max="9" defaultValue={vehiculo.plazas ?? ""} className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Puertas</label>
            <input name="puertas" type="number" min="1" max="5" defaultValue={vehiculo.puertas ?? ""} className={FIELD} />
          </div>
          <div className="col-span-2">
            <label className={LABEL}>Daños / observaciones</label>
            <input name="danios" defaultValue={vehiculo.danios ?? ""} placeholder="Ej: Pequeño roce en paragolpes trasero" className={FIELD} />
          </div>
        </div>
      </div>
      ...
      <div className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input name="precio" type="number" required min="1" step="0.01" defaultValue={precioEuros} className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Precio anterior (€) <span className="text-[var(--color-gris)] normal-case font-normal">tachado</span></label>
            <input name="precio_anterior" type="number" min="1" step="0.01" defaultValue={precioAntEuros} className={FIELD} />
          </div>
          <div>
            <label className={LABEL}>Estado <span className="text-red-400">*</span></label>
            <select name="estado" required className={FIELD} defaultValue={vehiculo.estado}>
          ...
          <div className="flex items-center gap-3 pt-6">
            <input
              ...
              name="destacado"
              type="checkbox"
              defaultChecked={vehiculo.destacado}
      ...
      {imagenes.length > 0 && (
        <div className={SECTION}>
          ...
          <div className={`grid grid-cols-4 gap-3 ${isDeleting ? "opacity-60 pointer-events-none" : ""}`}>
            {imagenes.map((img) => (
              <div key={img.id} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-[var(--color-negro-3)] group">
                <Image
                  src={img.url}
                  alt=""
                  fill
                ...
                />
                {img.es_principal && (
                  <span className="absolute bottom-1 left-1 text-[10px] bg-[var(--color-dorado)] text-black font-bold px-1.5 py-0.5 rounded z-10">
                    Principal
                  </span>
                )}
                <button
                  ...
                  className="absolute top-1 right-1 w-6 h-6 rounded-full bg-red-500/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10"
      ...
      <div className={SECTION}>
        <h2 className="text-sm font-semibold text-[var(--color-gris-claro)] uppercase tracking-wider mb-4">
          Añadir imágenes <span className="text-[var(--color-gris)] normal-case font-normal">(se añaden al final)</span>
        </h2>
